...
if ($_GET != '') {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $sqlRemitente = "SELECT usu_correo FROM usuario WHERE usu_correo LIKE'" . $_GET['key'] . "%' AND usu_codigo=" . $row["usu_remitente"] . ";";
            $resultRemitente = $conn->query($sqlRemitente);
            if ($resultRemitente->num_rows != 0) {
                while ($rowRemitente = $resultRemitente->fetch_assoc()) {
                    echo "<tr>";
